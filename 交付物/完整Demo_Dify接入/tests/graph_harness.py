...
import re
import yaml
from test_demo import ROOT,CAT,dump,envelope
...
class Harness:
    def __init__(self):self.receipts={};self.creates=[];self.mode='SUCCESS';self.trace=[]
    def execute(self,query,parsed,state=None,repair=None):
        doc=yaml.safe_load((ROOT/'差旅申请助手-完整Demo-可导入.yml').read_text());nodes={n['id']:n['data'] for n in doc['workflow']['graph']['nodes']};edges=doc['workflow']['graph']['edges']
        values={'sys':{'query':query,'workflow_run_id':'run-'+query},'conversation':{'cv_session':dump(state or {})},'env':{'DEMO_API_BASE_URL':'https://offline.invalid','DEMO_API_TOKEN':'offline-fixture'}}
        ...
        while pending:
            ...
            if typ=='start':out={'demo_reference_date':'2026-09-12'}
                ...
                if name=='N03':out={'structured_output':parsed}
                elif name=='R01':
                ...
                if name=='H01':out={'body':envelope({'employeeContext':CAT['employee_context'](),'transportOptions':CAT['transport_options'](),'cityNames':[c['data']['cityName'] for c in CAT['CITIES']]+['昆山']}),'status_code':200}
